...
@if($get == 'saved')
    <table class="messenger-list-item m-li-divider" data-contact="{{ Auth::user()->id }}">
        <tr data-action="0">
            {{-- Avatar side --}}
            <?php if(Auth::user()->type =='Patient'||  Auth::user()->type =='Consalutant' ){ ?>
                <td>
...
if($get == 'sharedPhoto'){
    $arrA = explode("/", $image);
$url='https://app.daleli.org/storage/attachments/'.$arrA[6];
?>
<div class="shared-photo chat-image" style="background-image: url('<?= $url ?>')"></div>
<?php } ?>
