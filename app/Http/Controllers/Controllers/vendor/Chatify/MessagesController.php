...
class MessagesController extends Controller
{
    ...
    {
        $ID_PATINT =session()->get('ID_PATINT');
      
        $consultant_id =session()->get('consultant_id');
        $User=User::find($ID_PATINT);
     
        $User->consultant_id= $consultant_id;
    ...
    {
        if(!empty(session()->get('ID_PATINT'))){
            return Message::where('from_id', session()->get('ID_PATINT'))->where('to_id', $user_id)
            ->orWhere('from_id', $user_id)->where('to_id', session()->get('ID_PATINT'));
        }else{
    ...
    public function fetch(Request $request)
    {
        ...
        $query = $this->fetchMessagesQuery($request['id'])->latest();
        ...
        foreach ($messages->reverse() as $index => $message) {
            $allMessages .= Chatify::messageCard(
                $this->fetchMessage($message->id, $index)
            );
        }
    ...
    public function fetchMessage($id, $index = null)
    {
        if(!empty(session()->get('ID_PATINT'))){
            ...
            return [
                ...
                'viewType' => ($msg->from_id == session()->get('ID_PATINT')) ? 'sender' : 'default',
                'seen' => $msg->seen,
            ];
        }else{
            $attachment = null;
